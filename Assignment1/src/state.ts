...
// the check line. for the targets, y grows downward, so closest to the line is
...
// check if the user input matches the base 16 target
const checkInput = (
...
/**
 * this is for the Punish to the player for missing mario, speeds up one random target on
...
// he does time out, that's a "miss", and miss Streak goes up by one and a fresh
// penaltySeed is hashed out so the next penalty (handles in applyMarioPenalty)
// picks an random random target and gets a bigger speed boost than
...
// a small curried functino so every Action that should freeze during 
// pause can just wrap its logic in whilePlaying(...) instead of every one
// of them repeating an `if (s.gameEnd) return s` guard
const guardBy =
...
// runs once per clock tick, basicaaly updating eberything
