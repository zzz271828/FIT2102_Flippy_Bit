...
/**
 * This file is the "model" of the game: the State type's starting value,
 * and one Action class per kind of thing that can happen (tick, spawn,
 * flip a bit, restart, etc). Every Action just has an apply(state) method
 * that returns a new state -- nothing here touches the DOM or RxJS, so it's
 * all easy to test and reason about on its own.
 */
...
// the check line -- targets above it don't matter yet
...
// random x/y for mario, kept fully inside the canvas
 ...
 * screen. The more times in a row mario is missed, the bigger the boost.
 */
...
// counts one tick forward, and expires mario if he's been on screen too
// long without being clicked (which then triggers the penalty above)
...
// runs a list of State -> State functions in order, left to right
const pipe =
    (...fns: ReadonlyArray<(s: State) => State>) =>
    (s: State): State =>
        fns.reduce((acc, f) => f(acc), s);

// wraps a State -> State function so it does nothing when `blocked` is true
// (used below to skip updates while paused/game over)
const guardBy =
...
// runs once per clock tick: move targets, check for a match, update mario,
// then check if a target has reached the line (game over)
...
// stream in observables.ts (see the comment there); everything else here
// (speed, whether mario also spawns) depends on state, so it's done here
...
// player clicked mario in time: removes the oldest target on screen as a
// reward, and resets the miss streak so the penalty stops growing
...
// throws away the current state and starts over, no page refresh needed
...
// the reducer: applies one Action to the current State to get the next one
const reduceState = (s: State, action: Action): State => action.apply(s);

/**
 * Scratch notes used while building this file, to check some hex values
 * against their binary digit positions by hand. Kept for reference.
 *
 * 23: 00100011 (3, 7, 8)
